...
for i in students:
    if i['course'] == 'python':
        python_s.append(i['name'])
    if i['course'] == 'javascript':
        js_s.append(i['name'])
    if i['course'] == 'java':
        java_s.append(i['name'])

courses = {'python': python_s,
           'javascript': js_s,
           'java': java_s
           }
...
names_age = {}
for i in students:
    names_age[i['name']] = i['age']
print(names_age)

# 3

names = ['Janyl', 'Nursultan', 'Meerim', 'Emir', 'Susann', 'Marcus', 'Aidin', 'Aigerim', 'Angela', 'Albert', 'Jyldyz', 'Doe', 'Gloria', 'Aliaskar',
         'Martin', 'John', 'Andrew', 'Steve', 'Johnathan', 'Adyl', 'Chyngyz', 'Michael', 'Atay', 'Mikkel', 'Agnes', 'Aidana', 'Sultan', 'Nash',
         'Nicolas', 'Mirbek', 'Aktan', 'Emirlan', 'Jennifer', 'Eniston', 'Alex', 'Mark']


def check_name(names, names_age):
    for i in names:
        try:
            print(f'{i} {names_age[i]} ')
        except KeyError:
            print(f'Такого имени нет')


check_name(names, names_age)
